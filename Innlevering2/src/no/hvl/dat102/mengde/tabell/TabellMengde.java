...
//	 }
	...
	public MengdeADT<T> union(MengdeADT<T> m2) {
		...
		while (teller2.hasNext()) {
			...
			((TabellMengde<T>) union).settInn(element);
		}
	...
	public MengdeADT<T> snitt(MengdeADT<T> m2) {
		...
		while (teller.hasNext()) {
			element = teller.next();
			if (this.inneholder(element)) {
				((TabellMengde<T>) snitt).settInn(element);
			}
		}
	...
	public MengdeADT<T> differens(MengdeADT<T> m2) {
		...
		while (teller.hasNext()) {
			element = teller.next();
			if (!this.inneholder(element)) {
				((TabellMengde<T>) differens).settInn(element);
			}
		}
	...
	public boolean undermengde(MengdeADT<T> m2) {
		boolean erEnUnderMengde = true;
		
		for(T element : m2) {
			if(!this.inneholder(element)) {
				erEnUnderMengde = false;
			}
		}
		
		return erEnUnderMengde;
	}
